...
# Retrieve the PostgreSQL versions for TimescaleDB-PostGIS-PGVector
cd ${BASE_DIRECTORY}/TimescaleDB-PostGIS-PGVector
for version in */; do
...
for version in "${postgis_versions[@]}"; do
	...
	entries+=(
		"{\"name\": \"TimescaleDB-PostGIS-PGVector ${version}-${postgisVersion}\", \"platforms\": \"$platforms\", \"dir\": \"TimescaleDB-PostGIS-PGVector/$version\", \"file\": \"TimescaleDB-PostGIS-PGVector/$version/Dockerfile\", \"version\": \"$version\", \"tags\": [\"$(join "\", \"" "${versionAliases[@]}")\"], \"fullTag\": \"${fullTag}\"}"
	)
done
